...
UDS server and DTC manager & 22 unit tests, M3 on-target self-test, M5 external tester & \cmark \\
Two-node physical CAN bus with transceivers, fault injection & M4 hardware test & \cmark \\
Python tester on Linux (SocketCAN) & -- & planned \\
Static analysis (cppcheck with MISRA C:2012 addon) & no findings, 6 documented deviations & \cmark \\
Demo video (3\,min) & published on YouTube & \cmark \\
\bottomrule
\end{tabularx}
\end{table}
