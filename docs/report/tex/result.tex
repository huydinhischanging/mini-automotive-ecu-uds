...
\section{Milestone M5: External UDS Tester}

The self-test of milestone M3 runs on the same core as the server and cannot exercise a reset. A small tester was therefore added to the Sensor ECU: it reuses the shared ISO-TP module (TX 0x7E0, RX 0x7E8), receives CAN frames through a ring buffer filled by the interrupt so that all ISO-TP processing stays in the main loop, and is driven by single-key commands on the ST-LINK virtual COM port. Table~\ref{tab:m5} lists the command sequences run over the physical bus; all 22 steps passed.

\begin{table}[H]
\centering
\small
\caption{Milestone M5: UDS from an external node}
\label{tab:m5}
\begin{tabularx}{\textwidth}{@{}clcX@{}}
\toprule
\textbf{Key} & \textbf{Sequence} & \textbf{Steps} & \textbf{Observation} \\
\midrule
t & full test & 12/12 & segmented request and response over the bus, NRC 0x11/0x31/0x7E, sessions, DTC read and clear \\
v & version and speed & 2/2 & speed read over UDS equals the speed the Sensor ECU publishes (161.4\,km/h) \\
d & read DTCs & 1/1 & e.g. U0100-87 and U0400-81, status 0x2E (pending, confirmed, test no longer failing) after the Sensor ECU was reflashed \\
f & functional 0x7DF & 2/2 & TesterPresent answered; unsupported service correctly silent \\
r & hard reset & 4/4 & positive response, restart, ECU answers again within 3\,s \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Hard reset on a core controlled by Linux}

The first hard-reset attempt returned the positive response 0x51 and then the ECU never answered again. Linux still reported the M4 as running, but its tick counters were frozen and \texttt{RCC\_MP\_GCR.BOOT\_MCU} read 0: \texttt{NVIC\_SystemReset()} had reset the Cortex-M4, which then stayed held in reset, because in production mode the boot of the M4 is controlled by Linux, and Linux was not told that the core had reset itself. Since a UDS hard reset is meant to emulate a power cycle of the ECU application, it was reimplemented as a firmware restart: interrupts are disabled, the peripherals owned by the M4 (FDCAN, TIM1, TIM7, UART7) are reset through the RCC, the core returns to the main stack in privileged mode, and execution jumps to the reset vector, so that RAM, the RTOS and all drivers are initialised again. With this change the ECU restarts in well under 3\,s and the sequence passes. Coordinating a true core reset with Linux (for example through the inter-processor communication controller) remains possible future work.

\section{Static Analysis}

All project-owned code (the portable modules in \texttt{common/} and the application and MCAL layers of both ECUs) was analysed with cppcheck~2.21 and its MISRA C:2012 addon, using the real HAL, CMSIS and FreeRTOS headers for type information; vendor code and STM32CubeMX-generated headers are parsed but not reported. The script \texttt{tools/static\_analysis.sh} makes the check reproducible and fails on any finding.

A first attempt reported many violations of rule 17.3 (implicit function declaration) and \textit{incomplete configuration} messages. They were an artefact of the setup: the CMSIS headers stop with \texttt{\#error Unknown compiler} unless the target compiler macros are defined, which silently hid every HAL declaration. After defining the GCC/ARM macros and adding the CMSIS-RTOS2 include path, the analysis saw the real types. Table~\ref{tab:misra} summarises how the remaining findings were handled.

\begin{table}[H]
\centering
\small
\caption{Handling of the static-analysis findings}
\label{tab:misra}
\begin{tabularx}{\textwidth}{@{}llX@{}}
\toprule
\textbf{Rule} & \textbf{Category} & \textbf{Resolution} \\
\midrule
12.2 & Required & fixed: 8-bit constant shifted by 8 in the UDS server, cast to 16 bit first \\
10.8 & Required & fixed: composite expressions cast to a wider type (ISO-TP and both applications) \\
17.7 & Required & fixed: unused return values of logging and mutex calls cast to \texttt{void} \\
11.8 & Required & fixed: \texttt{const} no longer cast away for the MP1 HAL transmit call (payload copied) \\
13.4, 17.8, 12.1, 15.4, 2.5 & Advisory & fixed: HAL macros replaced by register writes, parameter no longer modified, parentheses, single loop exit flag, unused macro used \\
15.5 & Advisory & deviation D1: early return for parameter checks \\
8.7, 2.5 & Advisory & deviation D2: public APIs of \texttt{common/} used by a separate analysis unit \\
8.9 & Advisory & deviation D3: file-scope module state, inspectable by symbol from Linux \\
11.4 & Advisory & deviation D4: CMSIS register access, MCAL layer only \\
21.6, 17.1 & Required & deviation D5: \texttt{printf}/\texttt{vprintf} debug log, application layer only \\
11.6 & Required & deviation D6: integer-to-pointer conversion of the vector table address for the software restart, one function \\
\bottomrule
\end{tabularx}
\end{table}

After these changes the analysis reports no findings. All unit tests still pass, and both firmwares were rebuilt and re-verified on the physical bus (100 frames/s, no E2E error). The deviation records are kept in \texttt{docs/misra\_deviations.md}. The cppcheck addon covers the automatically decidable rules only; it is not a certified checker and does not replace a review.
...
\begin{tabular}{@{}lrrl@{}}
\toprule
\textbf{Firmware} & \textbf{Code} & \textbf{RAM} & \textbf{Available} \\
\midrule
Sensor ECU (STM32G431) & 31.6\,KB & 2.5\,KB & 128\,KB flash / 32\,KB RAM \\
Control ECU (M4, \texttt{-O0}) & 76.5\,KB & 24.8\,KB & 128\,KB code / 128\,KB data SRAM \\
\bottomrule
\end{tabular}
\end{table}
